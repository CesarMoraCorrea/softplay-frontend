...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const MenuItem = ({ item }) => {
    ...
    return (
      ...
      >
        <item.icon className={`w-5 h-5 ${
          isItemActive ? 'text-white' : 'text-gray-500 dark:text-gray-400 group-hover:text-blue-500'
        }`} />
        <span className="font-medium">{item.label}</span>
      </Link>
    );
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full bg-white dark:bg-gray-800 shadow-2xl z-50 transition-transform duration-300 ease-in-out ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      } lg:translate-x-0 lg:static lg:shadow-none w-80 border-r border-gray-200 dark:border-gray-700`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full mt-1 ${
                ...
                {user?.role === 'admin_sistema' ? 'Administrador' : 
                 user?.role === 'admin_cancha' ? 'Admin Cancha' : 'Usuario'}
              </span>
            </div>
          </div>
        </div>
